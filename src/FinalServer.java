...
//        System.out.println(fl.getFreeblocks()+" "+fl.getFilename()+" "+fl.getNextfilename()+" "+fl.isInuse());
    ...
    public ResponsePacket writeInBlocks(RequestPacket request){
        ...
        for(i=0;i<numOfBlocks&&!fl1.getFilename().equals("null");i++){
            if(fl1.getmetadata().writeToFile(data[i])==-1){
...
//        MDS mds=new MDS(index,numOfBlockString);
        MDSPacket mdsPacket=new MDSPacket("putMapping",request.getFilename(), new MDS(index,numOfBlockString,file,nodeId));
        System.out.println("MDS sent: [ "+index+", "+numOfBlockString+", "+nodeId+", "+file+"]");
        int st=-1;
...
//                fileInputStream.read(bytes,0,64*1024);
    ...
    ResponsePacket deleteFile(RequestPacket req){
        MDS mds=req.getMds();
//        FreeList fl1=new FreeList(this.port,mds.getAddress());
        int num=Integer.parseInt(mds.getNum());
        int status=deallocate(mds.getAddress(),num);
        if(status==-1){
            return new ResponsePacket("deleteFile",0,status);
        }
        MDSPacket mdsPacket = new MDSPacket("dropMapping",req.getFilename(),false);
        try {
            mdsOut.writeObject(mdsPacket);
            MDSPacket mdsPacket1=(MDSPacket) mdsIn.readObject();
            if(mdsPacket1.isSuccess()){
                return new ResponsePacket("deleteFile",0,1);
            }else{
                return new ResponsePacket("deleteFile",0,-1);
            }
        }catch(Exception e){
        ...
        while(true) {
            try {
                ...
                while (eof == 0) {
                    ...
                    //                System.out.println(request.getCommand()+" "+request.getFilename()+" "+request.getNumOfBlocks()+" "+);
